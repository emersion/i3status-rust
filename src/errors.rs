...
pub trait OptionExt<T> {
    ...
    fn config_error<M: Into<ErrMsg>>(self, msg: M) -> Result<T>;
}
...
impl<T> OptionExt<T> for ::std::option::Option<T> {
    ...
    fn config_error<M: Into<ErrMsg>>(self, msg: M) -> Result<T> {
        self.ok_or_else(|| Error::Config(msg.into()))
    }
}
...
impl Error {
    ...
    pub fn config<M: Into<ErrMsg>>(msg: M) -> Self {
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::InBlock(block, error) => match error.as_ref() {
                Self::Config(msg) => write!(f, "Configuration error in block '{block}': {msg}"),
                _ => write!(f, "Error in block '{block}': {error}"),
            },
